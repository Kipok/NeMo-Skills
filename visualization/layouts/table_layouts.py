...
def get_sorting_layout(
    id: int = -1, available_params: List[str] = []
) -> html.Div:
        ...
        dbc.Row(
            ...
            + [
                get_models_selector_table_cell(
                    get_available_models(), name, i, i != 0
                )
...
def get_detailed_answers_rows(
    keys: List[str], colums_number: int
) -> List[dbc.Row]:
    return [
        dbc.Row(
            [
...
def get_general_stats_layout(
    ...
    return [
        html.Div(
            [html.Div(f'{name}: {value}') for name, value in stats.items()]
        )
    ]
