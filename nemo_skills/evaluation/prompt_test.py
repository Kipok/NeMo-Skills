...
tokenizer = CodeLlamaTokenizer.from_pretrained(
    model_name, trust_remote_code=True, suffix_first=False)


def generate_one_completion(pre, suf):
    # prompt = pre+"<FILL_ME>"+suf
    prompt = "▁<PRE>" + pre + "▁<SUF>" + suf + "▁<MID>"
    input_ids = tokenizer(prompt, suffix_first=False, return_tensors="pt")[
        "input_ids"].to('cuda')
...
example = {"task_id": "RandomSpanInfilling/HumanEval/0/1", "entry_point": "has_close_elements", "prompt": "from typing import List\n\n\ndef has_close_elements(numbers: List[float], threshold: float) -> bool:\n    \"\"\" Check if in given list of numbers, are any two numbers closer to each other than\n    given threshold.\n    >>> has_close_elements([1.0, 2.0, 3.0], 0.5)\n    False\n    >>> has_close_elements([1.0, 2.8, 3.0, 4.0, 5.0, 2.0], 0.3)\n    True\n    \"\"\"\n    for idx, elem in enumerate(numbers):\n        for idx2, elem2 in enumerate(numbers):\n            if", "suffix": " distance < threshold:\n\t\t\t\t\treturn True\n\n\treturn False\n", "canonical_solution": " idx != idx2:\n                distance = abs(elem - elem2)\n                if",
