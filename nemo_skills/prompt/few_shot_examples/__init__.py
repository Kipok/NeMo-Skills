...
for examples in examples_map.values():
    for example in examples:
        ...
        if 'solution' in example:
            example["solution"] = example["solution"].replace("{start_code}", CODE_SEPARATORS[0])
            example["solution"] = example["solution"].replace("{end_code}", CODE_SEPARATORS[1])
            example["solution"] = example["solution"].replace("{start_code_output}", CODE_OUTPUT_SEPARATORS[0])
            example["solution"] = example["solution"].replace("{end_code_output}", CODE_OUTPUT_SEPARATORS[1])
