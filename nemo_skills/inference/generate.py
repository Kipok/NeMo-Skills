...
class GenerateSolutionsConfig:
    ...
    def __post_init__(self):
        ...
        if self.server["server_type"] not in ["mamba_openai", "nemo_openai", "openai"] and self.prompt_template is None:
            raise ValueError("Prompt template is required for non-OpenAI servers")

        if self.server["server_type"] in ["mamba_openai", "nemo_openai", "openai"] and self.prompt_template is not None:
