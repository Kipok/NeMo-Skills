...
# TODO: For QWen we only support VLLM, but for own RMs we will be able to support everything
...
def rm_scoring(
    ...
    expname: str = typer.Option("rm-scoring", help="Nemo run experiment name"),
    ...
    server_type: SupportedServers = typer.Option(SupportedServers.vllm, help="Type of server to use"),
    ...
    """Score input-output pairs using LLM-based RM.

    Run `python -m nemo_skills.inference.rm_scoring --help` for other supported arguments
    (need to be prefixed with ++, since we use Hydra for that script).
    """
    setup_logging(disable_hydra_logs=False)
    extra_arguments = f'{" ".join(ctx.args)}'
    LOG.info("Starting RM Scoring job")
