...
def score_rm(
    ...
    input_dir: str = typer.Option(..., help="Where to pull inputs from"),
    output_dir: str = typer.Option(..., help="Where to put results"),
    expname: str = typer.Option("score_rm", help="Nemo run experiment name"),
    ...
    """Infer Reward Model scores for a given input file.

    Run `python -m nemo_skills.inference.score_rm --help` for other supported arguments
    ...
    get_server_command_rm = partial(get_server_command, nemo_server_start_template=nemo_rm_server_start_template)
        ...
        if num_random_seeds:
            ...
            for seed in range(starting_seed, starting_seed + num_random_seeds):
                ...
                add_task(
                    ...
                    with_sandbox=False,
                    run_after=run_after,
                    get_server_command=get_server_command_rm,
                )
        else:
            assert dependent_jobs == 0
            cmd = get_rm_cmd(
